...
import { LayoutDashboard, Building2, Store, CalendarClock, Users, BarChart3, Compass } from "lucide-react";
...
const ITEMS = [
  ...
  { href: "/admin/waiting", label: "대기", icon: Users },
...
export function AdminSidebar() {
  ...
  return (
    <aside className="sticky top-0 hidden h-dvh w-60 shrink-0 flex-col border-r border-border bg-card px-3 py-5 md:flex">
      <Link href="/admin" className="mb-6 flex items-center gap-2 px-2 text-lg font-extrabold">
        <span className="flex size-7 items-center justify-center rounded-lg bg-primary text-primary-foreground">
          <Compass className="size-4.5" />
        </span>
        Roam <span className="text-sm font-semibold text-muted-foreground">Admin</span>
      </Link>
