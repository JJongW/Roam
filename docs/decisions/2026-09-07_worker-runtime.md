...
처음엔 "맥미니가 죽었을 때 그 판단을 맥미니가 하면 안 된다"며 Vercel Cron 15분
주기로 넣었다. **두 가지가 틀렸다.**

① Vercel Hobby 플랜은 cron이 **하루 1회**로 제한된다. 15분 주기를 넣은 커밋에서
배포가 통째로 실패했다(로컬 빌드는 통과 — 환경 제약이라 코드로는 안 보였다).

② 논리도 반쯤 틀렸다. **맥미니가 꺼져 있는 동안엔 잡을 되돌려봐야 처리할 워커가
없다.** 회수가 실제로 값을 하는 건 워커가 돌아와서 자기 유령 잡을 치우는 순간이다.

그래서 회수의 본체를 **워커 기동 시점**으로 옮겼다(`worker/index.ts`). Cron은
하루 한 번 남겨두되 역할이 다르다 — **워커가 영영 안 돌아올 때 상태를 정직하게**
만든다. 그때 admin은 "running 3건"이 아니라 "queued 3건, 아무도 안 집는 중"을
봐야 한다.
