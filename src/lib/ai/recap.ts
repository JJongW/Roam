...
/** LLM 없거나 실패 시 결정론 폴백 서술. 항상 자연스러운 한 마디를 보장. */
export function fallbackNarrative(digest: VisitDigest): string {
  const themes = digest.themesEngaged.slice(0, 2).join("·");
  const base = `오늘 ${digest.boothsVisited.length}곳을 둘러봤어`;
  const themePart = themes ? ` — ${themes} 중심으로 알찬 관람이었어` : "";
  return `${base}${themePart}. 충분히 즐긴 하루였길 바라. 다음엔 네 취향을 더 잘 챙겨줄게.`;
}
