...
import { interestScore, rankBooths, scoreBooth, waitingPenalty, eventBoost, distance } from "./scoring";
...
const pref: Pick<UserPreference, "visitPurpose" | "interests" | "availableMinutes"> = {
...
describe("waitingPenalty", () => {
  it("is 0 when disabled or missing", () => {
    ...
    expect(waitingPenalty({ boothId: "b1", enabled: false, queueCount: 0, estimatedMinutes: 30, updatedAt: "" } as Waiting)).toBe(0);
  });
  it("caps at 1 for long waits", () => {
    expect(waitingPenalty({ boothId: "b1", enabled: true, queueCount: 100, estimatedMinutes: 60, updatedAt: "" })).toBe(1);
  });
});

describe("eventBoost", () => {
  const now = Date.parse("2026-06-10T10:00:00+09:00");
  const within: BoothEvent = { id: "ev", boothId: "b1", title: "", description: "", startTime: "2026-06-10T10:30:00+09:00", endTime: "2026-06-10T11:00:00+09:00" };
...
describe("scoreBooth + rankBooths", () => {
  const ctx = { preference: pref, waitingByBooth: {}, eventsByBooth: {}, now: Date.parse("2026-06-10T10:00:00+09:00") };

  it("scores a matching booth higher than a non-matching one", () => {
    const match = scoreBooth(booth({ id: "m", tags: ["ai"], popularity: 50 }), ctx);
    const miss = scoreBooth(booth({ id: "x", tags: ["food"], popularity: 50 }), ctx);
  ...
  it("penalizes long waiting", () => {
    const ctxWait = { ...ctx, waitingByBooth: { w: { boothId: "w", enabled: true, queueCount: 99, estimatedMinutes: 60, updatedAt: "" } } };
